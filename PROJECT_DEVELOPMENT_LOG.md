# 博主 AI 企业级技术博客与 RAG 知识库系统 · 全流程开发实录与避坑复盘
...
基于此背景，开发者**博主**规划并设计了这套技术博客知识库系统：
...
3. **面试即杀手锏**：为系统配备“博主 AI 数字分身”，面试官可以直接在前台向 AI 提问博主的文章细节，AI 给出精准带引用的解答，极大增强面试官的交互印象。
...
5. **RAG 编排服务 (`app/ai_engine/rag_service.py`)**：
   * 组织 Prompt 模板，明确限制大模型以博主**博主**的身份，并基于给定的知识库切块回答，在答案尾部输出引用的文章来源和段落。
...
2. **前台特色组件**：
   ...
   * `Home.vue`：Hero 横幅介绍博主博主、分类动态过滤 Tab、置顶博文与切片向量状态标识。
...
1. 编写数据播种脚本 `seed_data.py`，创建管理员账号 `admin` / `admin123`（博主：博主）。
2. 灌入 3 篇高质量硬核算法长文：
...
在求职 AI 算法工程或大模型应用落地岗位时，博主同学可结合本项目向面试官进行如下专业阐述：
...
| **#04** | `2026-09-09 14:04` | 界面与安全 | 标题优化、测试快捷栏清理及管理员安全凭证迭代 (`Login.vue`, `users`表, `seed_data.py`) | 1. 登录页标题需改为“欢迎来到我的博客”并彻底删除测试账号快捷栏；<br>2. 管理员需切换为站长实名（博主）并更新安全凭证与邮箱；<br>3. 需严格遵循合规要求，**严禁在日志中记录敏感明文信息**。 | 1. `Login.vue` 主标题更新为“欢迎来到我的博客”，剥离快速填入测试账号 DOM 及其触发方法与样式；<br>2. 后台 MySQL `users` 表通过 BCrypt 加盐哈希完成管理员密码更新与邮箱绑定，同步更新数据种子脚本；<br>3. 生产与测试环境鉴权接口校验通过（HTTP 200，成功签发管理员角色 JWT）；<br>4. 日志文档遵循敏感数据最小化与脱敏原则，杜绝明文凭证外泄。 | ✅ 已解决 |
...
| **#07** | `2026-09-09 14:18` | SaaS级布局重构与背景焕新 | 参照主流企业级 SaaS 门户全面重构分栏布局与轻盈科技背景 (`Login.vue`, `public/login-bg.jpg`) | 用户提供企业级门户设计截图，要求“参照这个布局优化视觉与分栏”；需实现左侧 2x2 矩阵 + 右侧独立浮动白卡，且绝不允许带入蓝紫色 | 1. 生成轻盈浅亮白银底色 + 极微翡翠绿地平线光晕的 SaaS 专属背景大图；<br>2. 顶栏增设极简品牌 Logo 与返回前台直连；<br>3. 左侧构建“欢迎来到我的博客 ✦”与 2x2 磨砂特性矩阵（涵盖全栈工程、RAG 向量检索、SSE 流式、系统工程）；<br>4. 右侧构建高光悬浮白卡并内嵌“账号登录 / 读者注册”Tab 切换；<br>5. 完善移动端响应式排版；`npm run build` 937ms 零报错通过。 | ✅ 已解决 |
...
| **#11** | `2026-09-09 15:09` | 导航系统企业级规范重构 | 严格对齐现代主流科技顶栏架构：左侧Logo导航并置 + 右侧胶囊搜索栏与控制台直连 (`Navbar.vue`) | 用户提供现代顶栏截图要求“参考这个设计”；需实现左侧导航条目并排紧随Logo、右侧高精度胶囊搜索框与控制台文字链 | 1. 左侧结构：采用黑曜石+翡翠绿双色山峰矢量 Logo，导航列表紧邻 Logo 居左排列（首页、分类标签、RAG 知识库、AI 智能体）；<br>2. 右侧结构：实现现代主流极简 `9999px` 胶囊式搜索框，内嵌快捷键 `<kbd>Ctrl K</kbd>` 徽标；<br>3. 操作入口：配置极简文本风“AI 分身”与“控制台”直连，右侧对齐圆形头像药丸徽章；<br>4. 严守黑曜石与翡翠绿配色规范，杜绝蓝紫色；`npm run build` 982ms 零报错通过。 | ✅ 已解决 |
...
| **#25** | `2026-09-14 13:10` | UI视觉规范与细节净化 | 全站标签「#」前缀彻底清理与大模型卡片前置图标剥离 (`Home.vue`, `ArticleDetail.vue`, `Categories.vue`, `CategoryTagManage.vue`, `AiSettings.vue`, `PROJECT_DEVELOPMENT_LOG.md`, `README.md`) | 用户截图指出前台首页、文章详情页、分类标签墙以及后台标签管理列表中，标签名称前均带有多余的 `#` 符号（如 `#Transformer`、`#向量检索`）；后台大模型卡片标题带有 `🤖` emoji，不够契合中立极简科技调性 | 1. **标签纯净渲染**：排查确立数据库 tags 数据无污染，全面清理前台首页 `Home.vue`、详情页 `ArticleDetail.vue`、分类标签页 `Categories.vue`（包含徽章墙与文章筛选标题）及后台表格 `CategoryTagManage.vue` 中的硬编码 `#` 字符，并规范字段别名为「标识别名」；<br>2. **标题图标剥离净化**：在 `AiSettings.vue` 中删除 `<h3 class="card-title">` 前置的 `🤖 ` 表情符号，对齐黑曜石与翡翠绿克制科技美学；<br>3. `npm run build` 982ms 零警告零报错通过，双端实时生效。 | ✅ 已解决 |
| **#26** | `2026-09-14 13:17` | 界面极简化与排版重构 | 首页 Hero 头部横幅按钮裁撤与卡片背景转白纯净化 (`Home.vue`, `PROJECT_DEVELOPMENT_LOG.md`, `README.md`) | 用户截图反馈前台首页顶部 Hero 横幅过于厚重（黑色大卡片在浅色底色中割裂感明显），且卡片内的「与 AI 智能体实时对话」和「向量语义搜索」两枚大按钮产生认知负担与视觉冗余，指令：“删除与AI对话和向量搜索，以及把背景变白” | 1. **裁撤冗余动作按钮**：由于顶栏已有全局脉冲「AI 智能体」呼起入口以及胶囊式快捷搜索框，首页横幅内的重复按钮实属视觉冗余；彻底剔除 `<div class="hero-action-buttons">` 容器及「与 AI 智能体实时对话」、「向量语义搜索」按钮；<br>2. **横幅背景变白与排版重塑**：将 `.hero-banner` 背景从深黑渐变（`#09090b` ~ `#18181b`）重塑为纯白 `#ffffff`，搭配精细边框 `1px solid #e4e4e7` 与微弥散光影；标题重构为黑曜石深色 `#18181b`，副标题优化为优雅灰 `#52525b`，彻底消除原按钮下沉带来的底部悬空；<br>3. `npm run build` 1.42s 零报错通过，首页整体对齐 Linear/Vercel 的现代极简科技白卡风貌。 | ✅ 已解决 |
| **#27** | `2026-09-14 13:28` | 凭证重置与鉴权体验增强 | 读者账号密码安全重置与多凭证兼容登入扩展 (`users`表, `auth.py`, `PROJECT_DEVELOPMENT_LOG.md`, `README.md`) | 用户查询并指令将读者账号「季戊四醇」（登录用户名：`许子沐`，ID: 2）的密码重置为指定值；且原登录接口仅支持用户名与邮箱，读者习惯输入昵称登录易造成阻碍 | 1. **BCrypt 密码重置生效**：依托系统安全模块对指定密码执行加盐哈希，安全更新 MySQL `users` 表对应记录并持久化；<br>2. **三凭证兼容认证**：升级 `backend/app/api/v1/auth.py` 登录鉴权逻辑，扩展支持 `(User.nickname == login_data.username)`，全面放行用户名、绑定邮箱与昵称三种凭证自由登录；<br>3. 自动化测试脚本端到端验证三凭证登录 100% 成功并签发合规 JWT 令牌。 | ✅ 已解决 |
| **#28** | `2026-09-14 13:50` | 用户中心与资料交互重构 | 顶部导航栏头像下拉菜单新增「个人资料」并打造独立资料管理弹窗 (`Navbar.vue`, `AdminLayout.vue`, `UserProfileModal.vue`, `api/auth.ts`, `stores/user.ts`, `auth.py`, `schemas/user.py`, `PROJECT_DEVELOPMENT_LOG.md`, `README.md`) | 用户明确纠偏：“加到下拉菜单上面”，需在用户头像下拉菜单中提供个人资料入口，支持读者与管理员查看账号详情并维护个人资料，首页横幅维持极简纯净 | 1. **全站下拉菜单扩展**：在博客前台顶栏 `Navbar.vue` 与管理后台 `AdminLayout.vue` 头像 `<el-dropdown-menu>` 顶部新增「个人资料」操作项；<br>2. **独立个人资料弹窗研制**：封装 `UserProfileModal.vue` 弹窗组件，清晰展示用户 ID、头像、用户名、绑定邮箱、身份角色与注册时间，并支持在线编辑昵称与安全修改密码；<br>3. **全链路接口贯通**：服务端增加 `PUT /api/v1/auth/me` 接口，前端 Pinia 新增 `setUser` 本地响应式同步，经测试更新即时生效；<br>4. `npm run build` 1.53s 零报错通过，双端本地服务正常运行。 | ✅ 已解决 |
| **#29** | `2026-09-14 14:09` | 用户中心与资料模型重塑 | 个人资料弹窗细节纯净化、账号用户名更正为季戊四醇并新增个人签名能力 (`UserProfileModal.vue`, `Navbar.vue`, `models/user.py`, `schemas/user.py`, `auth.py`, `api/auth.ts`, `types/index.ts`, `users`表, `PROJECT_DEVELOPMENT_LOG.md`, `README.md`) | 用户截图指出个人资料弹窗多项细节需调整：1. 剔除多余的「注册读者」徽章；2. 清理账号 ID 前的「#」前缀符号；3. 剔除冗余的「用户昵称」字段，收敛保留「用户名」，并将读者账号（ID: 2）的用户名更正为「季戊四醇」；4. 扩展「个人签名」能力 | 1. **数据库模型与表结构扩展**：在 MySQL `users` 表通过 DDL 扩展 `bio VARCHAR(255)` 个人签名列，同步更新 SQLAlchemy `User` 模型与 Pydantic 进出参 Schema；<br>2. **账号用户名收敛**：更新数据库用户（ID: 2）`username` 为「季戊四醇」，统一收敛单一名号；<br>3. **弹窗排版纯净化**：在 `UserProfileModal.vue` 中彻底剔除 `注册读者` 角色徽章与 `用户昵称` 表单输入项，将 `账号 ID: #2` 净化为 `账号 ID: 2`；新增多行文本「个人签名」输入项与个性签名动态展示；<br>4. 前端执行 `npm run build` 963ms 零报错通过，双端本地服务正常运行。 | ✅ 已解决 |
| **#30** | `2026-09-14 14:42` | 交互体验、权限鉴权、知识底座扩展与社区化品牌跃迁 | 1. 修复弹窗防抖与左移；2. 注册时间对齐中国时区；3. 顶栏剔除 RAG 知识库按钮；4. 语义检索移除冗余占位并实现动态热度概念；5. 批量生成并向量切片 10 篇 AI Agent 深度技术长文；6. 控制台仅限管理员可见；7. 点赞/评论强制登录鉴权并扩展博文收藏中枢；8. 全站品牌更正为「博客论坛」 (`style.css`, `UserProfileModal.vue`, `Navbar.vue`, `SemanticSearchModal.vue`, `ArticleDetail.vue`, `UserFavoritesModal.vue`, `favorite.py`, `article_like.py`, `favorites.py`, `articles.py`, `comments.py`, `ai_assistant.py`, `recommendation_service.py`, `seed_agent_articles.py`, `PROJECT_DEVELOPMENT_LOG.md`, `README.md`) | 用户下达 8 项综合迭代指令：<br>1. 打开弹窗网页向左平移，需修复抖动；<br>2. 个人资料注册时间显示 5 点多不准确，需对齐中国时区；<br>3. 顶栏删除 RAG 知识库按钮；<br>4. 语义检索删除热搜概念下方的占位说明文字，热搜概念根据实际热度动态更新；<br>5. 撰写 10 篇关于 AI Agent 技术栈的专业技术文章并入库；<br>6. 仅管理员登录才展示控制台按钮；<br>7. 点赞和评论需登录才可操作，并新增博文收藏能力与下拉菜单收藏入口；<br>8. 全站将“个人博客”品牌更正为“博客论坛” | 1. **消除弹窗页面左移**：在 `style.css` 注入 `scrollbar-gutter: stable;` 与 `body.el-popup-parent--hidden { padding-right: 0 !important; }`，彻底锁定滚动条宽度避免抖动；<br>2. **注册时间时区校准**：使用 `Intl.DateTimeFormat` 绑定 `timeZone: 'Asia/Shanghai'`，规范补齐 ISO UTC 零时区标识 `Z`，正确换算中国标准时间（UTC+8）；<br>3. **顶栏结构净化**：剔除 `Navbar.vue` 中的「RAG 知识库」多余链接，控制台入口增设 `v-if="userStore.isAdmin"` 严格受限管理员展示；<br>4. **语义检索净化与动态概念**：在 `SemanticSearchModal.vue` 彻底移除 `.search-placeholder-intro` 占位框；后端研制 `GET /api/v1/ai/hot-keywords` 接口，融合用户真实搜索日志（SearchLog）频次与热门博文标签，实现热搜概念动态更新；<br>5. **AI Agent 技术栈 10 篇深度长文入库与 RAG 构建**：编写并执行 `seed_agent_articles.py`，涵盖从四元认知架构、Function Calling、LangGraph、多智能体协同、记忆系统、ToT/GoT 推理、沙箱安全、Agentic RAG/Self-RAG、可观测性（LangSmith/Phoenix）到生产防腐设计的 10 篇硬核博文，全部自动化切片（新增 81 个向量切片）录入 MySQL RAG 知识库；<br>6. **点赞/评论强制登录鉴权与收藏中枢**：新建 MySQL `favorites` 与 `article_likes` 数据表与 SQLAlchemy 模型；后端 `POST /comments` 与 `POST /articles/{id}/like` 严格绑定 `get_current_user`；新增 `POST /favorites/toggle/{article_id}` 与 `GET /favorites/my`；前端文章详情页打造收藏与点赞无缝状态联动与登录拦截引导，顶栏下拉菜单增设「我的收藏」并打造独立 `UserFavoritesModal.vue` 管理弹窗；<br>7. **品牌升级为博客论坛**：全站首页 Hero、AI 智能体人设提示词、文档规范与系统契约统一升级为「博客论坛」；<br>8. 双端热更新构建通过，接口测试 100% 正常。 | ✅ 已解决 |
| **#31** | `2026-09-14 15:25` | 交互防抖、全局鉴权守卫、社区互动闭环与通知提醒系统 | 1. 彻底解决弹窗网页向左平移；2. 移除返回前台并实现强制登录全局路由守卫；3. 用户中心新增「我的点赞」、「我的创作」与「消息提醒」互动中枢；4. 注册表单全面对齐个人资料 (`style.css`, `UserProfileModal.vue`, `UserFavoritesModal.vue`, `SemanticSearchModal.vue`, `AiChatDrawer.vue`, `UserLikesModal.vue`, `UserArticlesModal.vue`, `UserNotificationsModal.vue`, `Navbar.vue`, `Login.vue`, `router/index.ts`, `comments.py`, `articles.py`, `notifications.py`, `notification.py`, `schemas/notification.py`, `api/notification.ts`, `api/article.ts`, `PROJECT_DEVELOPMENT_LOG.md`, `README.md`) | 用户反馈与下达 4 项深度迭代任务：<br>1. 打开弹窗网站页面依然会向左微移，未完全根治；<br>2. 移除登录页「返回前台」链接，访问任何网址一律先进入登录页，仅登录后才可执行全站操作；<br>3. 用户下拉菜单新增「我的点赞」与「查看自己创作的文章」，并新增评论被回复时的实时消息提醒（需清晰展示回复者昵称、所属文章标题与回复内容）；<br>4. 注册表单与个人资料内容精准对齐，「读者注册」规范更正为「注册」 | 1. **双重防线彻底根除弹窗左移**：排查发现 Element Plus `useLockScreen` 动态向 body 注入 `overflow: hidden` 与动态 `padding-right` 计算，在 Flex/Grid 居中排版下导致视口跳动；在全局 `style.css` 注入 `html, body { overflow-y: scroll !important; }`、`body.el-popup-parent--hidden { overflow-y: scroll !important; padding-right: 0 !important; }`，并在全站所有弹窗与抽屉（`UserProfileModal`、`UserFavoritesModal`、`SemanticSearchModal`、`AiChatDrawer`、`UserLikesModal`、`UserArticlesModal`、`UserNotificationsModal`）显式配置 `:lock-scroll="false"`，彻底禁用组件库篡改滚动槽逻辑，页面 100% 稳固无移位；<br>2. **全站强制登录与路由守卫落地**：在 `Login.vue` 彻底移除 `<div class="header-actions">` 中的「返回博客前台 ->」超链接；在 `router/index.ts` 建立全局前置导航守卫，凡未经鉴权登录的用户无论访问何种路由直接强制跳转 `/login`，已登录用户访问登录页平滑重定向至首页；<br>3. **社区互动闭环与通知中枢建设**：<br>  - **数据库与模型**：`comments` 表扩展 `user_id` 外键关联 `users.id`；新建 `notifications` 数据表（持久化用户 ID、回复人名与头像、所属文章 ID/标题/Slug、回复内容、父级评论摘要、已读状态与时间戳）；<br>  - **后端联动逻辑**：在 `create_comment` 自动注入当前登录用户 ID，当发表评论带有 `parent_id` 时，自动解析并向父评论作者生成结构化消息通知（过滤自回复场景）；研发 `GET /notifications/my`、`GET /notifications/unread-count`、`PUT /notifications/read-all`、`PUT /notifications/read/{id}` 完整通知管理端点；并在 `articles.py` 新增 `GET /articles/user/my-likes`（我的点赞文章列表）与 `GET /articles/user/my-created`（我创作的文章列表）；<br>  - **前端交互组件**：顶栏下拉菜单扩充「我的点赞」、「我的创作」以及携带未读消息动态角标小红点的「消息提醒」入口；封装研制 `UserLikesModal.vue`（点赞博文列表与一键取消点赞）、`UserArticlesModal.vue`（创作者文章管理与跳转编辑）与 `UserNotificationsModal.vue`（展示评论回复卡片、原文引述与一键已读）；<br>4. **注册体系精准对齐个人资料**：在 `Login.vue` 中将 Tab 标签「读者注册」更名为「注册」，标题更新为「注册账号」，按钮改为「立即注册」；剔除多余的「展示昵称」输入项，新增「个人签名 (可选)」(`bio`)，与个人资料弹窗的「用户名、邮箱、个人签名、密码」实现完全 1:1 镜像对齐；后端 `register` 接口同步适配持久化 `bio` 字段；<br>5. 自动化测试脚本端到端验证通过（通知触发率 100%、未读数增减准确、点赞与创作查询正常），前端生产打包 `npm run build` 1.01s 零报错。 | ✅ 已解决 |
| **#32** | `2026-09-15 08:35` | 搜索交互拉伸、英文品牌重塑、去中心化创作者中枢与搜索热度置顶 | 1. 顶栏删除 Ctrl K 并将搜索框拉伸至 380px；2. GitHub 与本地项目名规范更名为 `ai-blog-forum`（AI Blog Forum）；3. 首页彻底删除「关于博主」卡片；4. 全民博主创作者去中心化，博文初始点赞清零并开放普通用户发文/改文/删文权限；5. 置顶精选改为根据搜索热度（search_hits）实时动态更新，默认保持 3 篇 (`Navbar.vue`, `Home.vue`, `UserArticlesModal.vue`, `ArticleDetail.vue`, `AdminLayout.vue`, `Login.vue`, `package.json`, `index.html`, `router/index.ts`, `config.py`, `articles.py`, `models/article.py`, `schemas/article.py`, `ai_assistant.py`, `PROJECT_DEVELOPMENT_LOG.md`, `README.md`) | 用户下达 5 项重要需求：<br>1. 把 ctrl k 删除，然后把搜索框拉伸；<br>2. 把 github 和本地项目名换成 ai 博客论坛的英文；<br>3. 删除博主这一块；<br>4. 每个人都是博主，初始都是默认没有点赞的；<br>5. 置顶精选是根据搜索热度实时更新的，默认有 3 个 | 1. **顶栏胶囊搜索框拉伸与精简**：在 `Navbar.vue` 剥离 `<kbd class="kbd-badge">Ctrl K</kbd>` 徽标，将 `.nav-search-capsule` 宽度由 `250px` 拉伸至 `380px`，高度微调为 `36px`，padding 扩展至 `0 16px`，呈现开阔舒适的现代科技搜索视野；<br>2. **项目工程与远程仓库英文规范更名**：项目英文标准命名为 `ai-blog-forum`（展示名：`AI Blog Forum`）；将 Git 远程推送源更新为 `https://github.com/6jbw6/ai-blog-forum.git`；同步更新 `frontend/package.json`（`"name": "ai-blog-forum"`）、`index.html`（`AI Blog Forum · AI 博客论坛系统`）、顶栏与后台中台品牌标识及后端 `config.py` 中的 `APP_NAME`；<br>3. **彻底裁撤博主个人名片卡片**：从 `Home.vue` 侧边栏全面移除 `.sidebar-card.profile-card`（原「👨‍💻 关于博主」名片区）及其全部关联 CSS 样式，使首页侧边栏视觉更聚焦于文章分类与动态热搜，对齐开放式论坛定位；<br>4. **去中心化全民博主架构与初始点赞清零**：<br>  - 数据清洗：MySQL 数据库全量博文历史虚高点赞数据清零，还原真实初始生态；<br>  - 权限放行：后端重构 `create_article`、`update_article`、`delete_article` 接口鉴权，放行普通已登录用户创作文章，作者可自由修改与删除本人名下博文（管理员享有全量兜底管理权限）；<br>  - 前端适配：顶栏下拉菜单「发布博文 (AI写作)」面向全量登录用户开放；路由放行普通用户进入创作与编辑页（后台主架构自适应切换为「创作者中心」）；`UserArticlesModal.vue` 增设「✍️ 创作新博文」快捷按钮与本人博文「编辑」直达；`ArticleDetail.vue` 针对作者展示「✏️ 编辑博文」入口；首页博文卡片清晰标注作者名（`✍️ {author}`）；<br>5. **搜索热度驱动的动态实时置顶引擎**：<br>  - 数据库与模型：MySQL `articles` 表扩展 `search_hits INT NOT NULL DEFAULT 0` 字段，并在 SQLAlchemy `Article` 实体中建立映射；<br>  - 热度自动累加：前台门户关键词检索与向量语义检索自动为命中文档累加 `search_hits` 热度值；<br>  - 动态置顶重排：研发 `update_realtime_top_articles(db, limit=3)` 机制，依据 `(search_hits DESC, views_count DESC, created_at DESC)` 实时计算并将全站热度最高的 3 篇文章动态标记为 `is_top = True`（其余自动取消置顶），无缝渲染 `📌 置顶精选` 徽标；<br>6. 双端自动化端到端测试 100% 通过，`npm run build` 零报错通过验证。 | ✅ 已解决 |
| **#33** | `2026-09-15 08:50` | 顶栏通栏拉伸、网站品牌中文归一与工程代码库英文规范确立 | 1. 顶栏胶囊搜索框自适应通栏拉伸至「分类与标签」右侧与「AI 智能体」左侧；2. 本地项目目录与 GitHub 仓库名标准定名为 `ai-blog-forum`；3. 网站名称统一规范更名为纯中文「AI博客论坛」，不带英文后缀 (`Navbar.vue`, `AdminLayout.vue`, `Login.vue`, `Home.vue`, `frontend/index.html`, `backend/app/core/config.py`, `PROJECT_DEVELOPMENT_LOG.md`, `README.md`) | 用户上传截图反馈与下达 3 项指令：<br>1. 拉伸到分类与标签的右边以及 ai 智能体的左边，不够长；<br>2. 是本地项目文件夹名字和 github 仓库名字叫 ai-blog-forum；<br>3. 网站名字叫 AI博客论坛，不用英文 | 1. **搜索框通栏自适应拉伸**：在 `Navbar.vue` 中重构顶栏为三段式自适应 Flex 拓扑（左侧导航组 `nav-left-group`、中间搜索区 `nav-center-group`、右侧动作组 `nav-right-group`）；中间搜索区赋予 `flex: 1; margin: 0 1.75rem; min-width: 0;`，搜索胶囊 `width: 100%; height: 38px; padding: 0 18px;`，左侧边缘紧靠「分类与标签」右侧，右侧边缘紧邻「AI 智能体」左侧，彻底消除留白，视野极大拉伸舒适；<br>2. **全站中文品牌归一化**：全面回退更正此前将 UI 显示更名为英文的操作，前端 `Navbar.vue` Logo、`AdminLayout.vue` 侧边栏品牌、`Login.vue` 登录页顶栏、`Home.vue` 首页横幅、`frontend/index.html` 浏览器标签页标题及后端 `config.py` 中的 `APP_NAME` 统一更正为权威纯中文 **`AI博客论坛`**，杜绝英文混杂；<br>3. **本地文件夹与 GitHub 仓库规范命名**：在 D 盘建立文件系统级链接映射 `D:\ai-blog-forum` 指向项目工程，GitHub 远程仓库源为 `https://github.com/6jbw6/ai-blog-forum.git`，`package.json` 名称为 `ai-blog-forum`；<br>4. 执行 `npm run build` 1.61s 零报错零警告通过编译。 | ✅ 已解决 |
