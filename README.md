...
> **项目名称**：AI博客论坛 (`ai-blog-forum`) · 去中心化 AI 技术博客与社区论坛系统  
...
- **去中心化全民博主创作架构 (Decentralized Multi-Author Platform)**：
  - 彻底打破单博主单边发布模式，每位注册用户天然皆为独立博主与创作者；
  - 权限体系全面放行：已登录用户均可直接在线创作发布、编辑修改及删除属于本人的博文，管理后台自适应切换为「创作者中心」；
  - 文章数据全面清退虚高刷赞，点赞生态归零真实起步；卡片动态呈现博文创作者身份。
- **搜索热度驱动的实时动态置顶引擎 (Real-time Search-Heat Top Articles)**：
  - 数据库设计 `search_hits` 指标，在前台门户关键词检索与 RAG 向量语义搜索时自动累加命中文档热度；
  - 服务端动态计算全站热度排序 `(search_hits DESC, views_count DESC, created_at DESC)`，排名前 3 的热门文章自动晋升并实时加冕 `📌 置顶精选`，彻底告别后台人工硬编码置顶。
- **开阔胶囊搜索与界面视觉净化 (Stretched Search & Clean UI)**：
  - 顶栏胶囊搜索框宽度拉伸至 `380px`，剥离多余的快捷键角标，带来现代化开阔搜索视野；
  - 首页侧边栏彻底裁撤个人主观宣传名片，全面聚焦论坛社区分类导航与动态热搜概念。
...
14. **去中心化全民博主架构与搜索热度置顶**：重构权限与内容生态为去中心化创作者论坛，每位注册用户均拥有博主身份，可自由发布、编辑与管理个人博文；数据库点赞数归零真实起步，保障社区公平性；结合门户关键词检索与语义向量检索的 `search_hits` 热度累加机制，研制 `update_realtime_top_articles` 动态置顶引擎，全站热度前 3 的博文自动加冕置顶精选，实现热度感知型社区运营。
