      ...
      <div class="content-layout">
        ...
        <section class="articles-section">
          ...
          <div class="category-tabs-bar">
            <button
              :class="['tab-btn', currentCategoryId === null ? 'active' : '']"
            ...
            <button
              ...
              :key="cat.id"
              :class="['tab-btn', currentCategoryId === cat.id ? 'active' : '']"
          ...
          <div v-else-if="articles.length > 0" class="articles-list">
            ...
            >
              <div class="card-header">
                ...
                <span v-if="art.category" class="cat-tag">{{ art.category.name }}</span>
          ...
          <div v-else class="empty-box">
            <el-empty description="当前分类下暂无文章" />
          </div>
        </section>
...
const loadData = async () => {
  loading.value = true
  try {
    const [artsRes, catsRes, tagsRes] = await Promise.all([
      getArticlesApi({ page: 1, size: 20, category_id: currentCategoryId.value || undefined }),
      getCategoriesApi(),
      getTagsApi()
    ])
    articles.value = artsRes.list
    totalArticles.value = artsRes.total
...
const filterByTag = (tagId: number) => {
  getArticlesApi({ tag_id: tagId }).then(res => {
...
.tab-btn.active {
  ...
  color: #ffffff;
  border-color: #18181b;
  box-shadow: 0 4px 10px rgba(24, 24, 27, 0.2);
}
